...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const penaltyScenarios = [
    ...
    {
      s: "A robot launches a game element out of the field perimeter.",
      a: "Penalty",
    },
    {
      s: "A human player introduces a game element through the designated return chute.",
      a: "Legal",
    },
    {
  ...
  // Using Llama-3.1-8B-Instruct but quantized for browser
...
- The FIRST organization globally, including FIRST Lego League (FLL), FIRST Tech Challenge (FTC), and FIRST Robotics Competition (FRC).
- Robotics in general (hardware, software, strategy, outreach).
- Team 19639 Alphabots (based in Fremont, CA, 501c3 nonprofit).

If a user asks about anything completely unrelated (e.g., asking for recipes, writing poems about dogs, general world history), you must politely decline and state that you are specifically designed to talk about FIRST Robotics and the Alphabots.
Be concise, helpful, and enthusiastic. Use formatting (like bolding) to make your answers readable.`;
  ...
  async function handleSend() {
    ...
    try {
      // First LLM Call (might return text, or might call a tool)
      ...
      if (msg.tool_calls && msg.tool_calls.length > 0) {
        for (const toolCall of msg.tool_calls) {
          if (toolCall.function.name === "get_ftc_team_stats") {
            chatStatusText.textContent = "Fetching Stats...";
            const args = JSON.parse(toolCall.function.arguments);
            const result = await executeFtcScoutTool(args.team_number);

            // Let User know a tool was used (optional)
            addMessageToUI(
              "system",
              `*Used tool: fetched stats for Team ${args.team_number}*`,
            );
        ...
        // Call LLM again so it can read the tool result and answer the user
        chatStatusText.textContent = "Reading Stats...";
        response = await engine.chat.completions.create({
          messages: chatHistory,
          tools: tools, // Pass tools again just in case
        });
      ...
      addMessageToUI("system", "An error occurred during generation.");
    } finally {
      isGenerating = false;
      chatInput.disabled = false;
      chatSendBtn.disabled = false;
